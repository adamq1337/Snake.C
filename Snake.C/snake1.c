...
bool snake_hit_wall(snake_t *snake) {
    //coordinates y=1,Y_SIZE and x=1, X_SIZE are walls
    if (snake->x[0] < 1 || snake->x[0] > X_SIZE || snake->y[0] < 1 || snake->y[0] > Y_SIZE) {
        return true;
    } else {
        return false;
    }
...
void test() {
    ...
    test_snake_init();        //works
    test_snake_move();        //works
    test_snake_turn();        //works
    test_snake_hit_self(); // to pass, this needs snake_turn to be working too
    test_snake_eat_apple();   //works
